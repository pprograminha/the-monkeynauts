...
CREATE ROLE "${user_name}" WITH PASSWORD '${user_password}' NOBYPASSRLS NOSUPERUSER LOGIN;
ALTER ROLE "${user_name}" SET search_path TO themonkeynauts, extensions;
GRANT USAGE ON SCHEMA themonkeynauts, extensions TO "${user_name}";
GRANT SELECT, UPDATE, INSERT, DELETE, TRIGGER ON ALL TABLES IN SCHEMA themonkeynauts TO "${user_name}";

ALTER DEFAULT PRIVILEGES
    FOR USER "${superuser_name}"
    IN SCHEMA themonkeynauts
    GRANT SELECT, UPDATE, INSERT, DELETE, TRIGGER ON TABLES TO "${user_name}";

ALTER DEFAULT PRIVILEGES
    IN SCHEMA themonkeynauts, extensions GRANT EXECUTE ON ROUTINES TO "${user_name}";
...
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA themonkeynauts TO "${user_name}";
